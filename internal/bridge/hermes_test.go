...
func TestBridgeHermes_FreshSetup(t *testing.T) {
	withFakeHome(t, func(home string) {
		vault := t.TempDir()
		res, err := Bridge(HarnessHermes, vault, Options{
			DryRun:        false,
			Scope:         "shared",
			IncludeMemory: true,
		})
...
// rename pattern keeps working through the bridged symlink.
func TestBridgeHermes_AtomicRenameSafety(t *testing.T) {
	withFakeHome(t, func(home string) {
		vault := t.TempDir()
		if _, err := Bridge(HarnessHermes, vault, Options{
			DryRun:        false,
			Scope:         "shared",
			IncludeMemory: true,
		}); err != nil {
...
`
		...
		// Default = MCP only.
		if _, err := Bridge(HarnessHermes, vault, Options{DryRun: false}); err != nil {
			t.Fatalf("Bridge: %v", err)
		}
...
// TestBridgeHermes_MemoryOnlyWithSkipMCP verifies that --memory + --no-mcp
// produces only the memory bridge (no config edits).
func TestBridgeHermes_MemoryOnlyWithSkipMCP(t *testing.T) {
	withFakeHome(t, func(home string) {
		vault := t.TempDir()
		configPath := filepath.Join(home, ".hermes", "config.yaml")
		...
		if _, err := os.Stat(configPath); !os.IsNotExist(err) {
			t.Errorf("expected no config file with --no-mcp, but got %v", err)
		}
...
func TestUnbridgeHermes_RoundTrip(t *testing.T) {
	withFakeHome(t, func(home string) {
		...
		if _, err := Unbridge(HarnessHermes, vault, Options{
			DryRun:        false,
			Scope:         "shared",
			IncludeMemory: true,
		}); err != nil {
